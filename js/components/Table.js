    ...
    this.theadRow.innerHTML = this.columns
      .map((col) => {
        const sortIcon = col.sortable
          ...
          : '';
        return `<th class="${col.className || ''}" data-key="${col.key}" style="${col.align === 'right' ? 'text-align:right' : ''}">${col.label}${sortIcon}</th>`;
      })
      .join('');
  ...
  _render() {
    ...
    if (!filtered.length) {
      ...
      this.tbody.innerHTML = pageRows
        .map(
          (row) => `<tr>${this.columns
            .map((col) => `<td class="${col.className || ''} ${col.align === 'right' ? 'num' : ''} ${col.wrap ? 'wrap' : ''}">${col.render(row)}</td>`)
            .join('')}</tr>`
        )
        .join('');
    ...
    this.paginationInfo.textContent = filtered.length
      ? t('common.showingRange', { start: start + 1, end: Math.min(start + this.pageSize, filtered.length), total: filtered.length })
      : '';

    this.paginationControls.innerHTML = filtered.length
      ? `
        <button class="btn btn-ghost btn-sm" data-page="prev" ${this.page === 1 ? 'disabled' : ''}>${t('common.prev')}</button>
        <span class="text-secondary" style="padding:0 8px;">${this.page} / ${totalPages}</span>
        <button class="btn btn-ghost btn-sm" data-page="next" ${this.page === totalPages ? 'disabled' : ''}>${t('common.next')}</button>
      `
      : '';
